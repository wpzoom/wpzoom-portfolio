...
class WPZOOM_Blocks_Portfolio {
	...
	public function render( $attr, $content ) {
		...
		// Eccentric, Carousel and Metro are PRO-only layouts. When PRO is inactive
		// this fallback renderer runs, so degrade them gracefully to the grid (overlay).
		if( in_array( $layout, array( 'eccentric', 'carousel', 'metro' ), true ) ) {
			$layout = 'grid';
		}
		...
		// has no definite parent to resolve against.)
		$image_sizing = '';
				...
				elseif ( $args[ 'show_thumbnail' ] && ! empty( $thumbnail ) ) {
					...
					if( $args[ 'lightbox' ] ) {
						...
						$output .= "<span class='{$class}_lightbox_icon'>
										<svg xmlns='http://www.w3.org/2000/svg' height='24px' viewBox='0 -960 960 960' width='24px' fill='#fff'><path d='M140-140v-300h60v198.23L718.23-760H520v-60h300v300h-60v-198.23L241.77-200H440v60H140Z'/></svg>
									</span>";
						$output .= '</a>';
					};
